...
exports.getPendingDeliveries = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: 'فشل في جلب السندات المتأخرة',
      error: error.message
    });
  }
};
exports.getTodayDeliveriesByStatus = async (req, res) => {
  try {
    ...
    const uncollected = await CashDelivery.findAll({
      where: { ...baseWhere, is_collected: false },
      include: [
        { model: User, as: 'user', attributes: ['id', 'full_name'] },
        { model: Branch, as: 'branch', attributes: ['id', 'name_ar'] }
      ],
      attributes: ['id', 'delivery_number', 'delivered_amount', 'date', 'user_id', 'branch_id'],
      order: [['created_at', 'ASC']]
    });

    const collected = await CashDelivery.findAll({
    ...
    res.json({ collected, uncollected });
  } catch (error) {
