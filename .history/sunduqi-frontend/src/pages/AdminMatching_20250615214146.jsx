  ...
  const fetchMatchings = async () => {
    try {
      ...
      const url = selectedDate
        ? `/cash-matching?date=${selectedDate}`
        : `/cash-matching`;
      const res = await axios.get(url);

      if (res.status === 200 && Array.isArray(res.data?.data)) {
        const cashierOnly = res.data.data.filter(m => m.user?.role === 'casher');
  ...
  return (
    <div className="p-6 text-right font-[Tajawal]" dir="rtl">
      ...
      {loading ? (
        ...
        <MatchingTable
          matchings={activeTab === 'unresolved' ? unresolvedMatchings : resolvedMatchings}
          onResolve={activeTab === 'unresolved' ? handleResolve : null}
        />
      )}
    </div>
  );
    ...
    <div className="overflow-x-auto border rounded-lg shadow-sm mb-6">
      <table className="min-w-full table-auto text-sm text-right text-gray-800">
        ...
        <tbody>
          {matchings.map((m) => {
            const diff =
              (parseFloat(m.expected_total) || 0) -
              (parseFloat(m.actual_total) || 0);
            return (
