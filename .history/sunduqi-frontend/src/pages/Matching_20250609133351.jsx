// This React component handles cash matching functionality for both admin and cashier roles.
// Admins see all branches and input actual values; cashiers see their own branch's summary.
...
export default function Matching() {
  ...
  useEffect(() => {
    const loadSummary = async () => {
      try {
        ...
        console.error('❌ Failed to load matching summary:', error);
    ...
    const fetchMatchedTotal = async () => {
      try {
        ...
        console.error('❌ Failed to fetch matched total:', error);
      }
    };
  ...
  const handleMatchedDelivery = async () => {
    try {
      ...
      console.error('❌ Deliver matched error:', error);
              ...
              <tbody>
                {allBranches.map((branch) => {
                  ...
                  const difference = actual
                    ? (parseFloat(actual) - branch.expected_total).toFixed(2)
                    : '---';
                      ...
                      <td className="p-2 border">
                        <input
                          ...
                          onChange={(e) =>
                            setActualTotals({
                              ...actualTotals,
                              [branch.branch_id]: e.target.value,
                            })
                          }
