...
export default function AdminCollection() {
  ...
  const handleCollect = async (delivery) => {
    const payload = {
      ...
      total_collected: delivery.total_delivered,
      notes: `استلام من ${delivery.user.full_name} - رقم التسليم`,
    ...
    try {
      ...
      toast.error('❌ فشل في عملية الاستلام');
    }
  };

  const renderItem = (item, collected) => (
    <div key={`${item.user_id}-${item.branch_id}`} className="bg-white rounded-xl border shadow p-4 space-y-1">
      <div className="text-sm text-gray-800">
        ...
        <p><span className="font-semibold text-gray-600">المبلغ:</span>{' '}
          <span className="text-green-700 font-bold">
            {parseFloat(item.total_delivered).toLocaleString()} ₪
          </span>
        </p>
      </div>
      {!collected && (
  ...
  return (
    <div className="p-4 md:p-6 font-[Tajawal] text-right" dir="rtl">
      <h2 className="text-2xl font-bold text-yellow-600 mb-4">سندات التسليم</h2>
