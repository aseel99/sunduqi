...
export default function AdminMatching() {
  ...
  const handleResolve = async (id) => {
    try {
      ...
      fetchMatchings(); // إعادة التحميل بعد التحديث
    } catch (err) {
  ...
  // تصنيف المطابقات حسب حالة المراجعة
  ...
  return (
    <div className="p-6 text-right">
      ...
      {loading ? (
        ...
        <>
          {/* المطابقات التي لم تتم مراجعتها */}
          ...
          {/* المطابقات التي تمت مراجعتها اليوم */}
        ...
        {matchings.map(m => {
          ...
          return (
            <tr key={m.id}>
              ...
              <td className="border p-2">{(m.expected_total || 0).toFixed(2)}</td>
              <td className="border p-2">{(m.actual_total || 0).toFixed(2)}</td>
