...
export default function AdminCollection() {
  ...
  const handleCollect = async (delivery) => {
    const payload = {
      delivery_id: delivery.id, // ✅ نستخدم معرف السند فقط
  ...
  const renderDeliveryCard = (delivery, isCollected) => (
    <div key={delivery.id} className="bg-white rounded-xl border shadow p-4 space-y-2">
      <div className="text-sm text-gray-800 space-y-1">
        <p><span className="font-semibold text-gray-600">رقم التسليم:</span> {delivery.delivery_number}</p>
        <p><span className="font-semibold text-gray-600">المستخدم:</span> {delivery.user?.full_name}</p>
        <p><span className="font-semibold text-gray-600">الفرع:</span> {delivery.branch?.name_ar}</p>
        <p>
          <span className="font-semibold text-gray-600">التاريخ:</span> {delivery.date}
          <span className="ml-3 font-semibold text-gray-600">الوقت:</span> {formatTime(delivery.created_at)}
        </p>
        <p>
          <span className="font-semibold text-gray-600">المبلغ:</span>{' '}
          <span className="text-green-700 font-bold">{parseFloat(delivery.delivered_amount).toLocaleString()} ₪</span>
        </p>
      </div>
  ...
  return (
    <div className="p-4 md:p-6 font-[Tajawal] text-right" dir="rtl">
      <h2 className="text-2xl font-bold text-yellow-600 mb-4">سندات تسليم الكاش اليومية</h2>

      {/* فلتر التاريخ */}
      <div className="mb-4 flex gap-4 items-center">
        <label className="text-gray-700">تصفية بالتاريخ:</label>
        <input
          type="date"
          value={filterDate}
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {currentList.length > 0 ? (
            currentList.map(delivery => renderDeliveryCard(delivery, activeTab === 'collected'))
          ) : (
