...
export default function AdminCollection() {
  ...
  const [branches, setBranches] = useState([]); // ✅ مهيئة كمصفوفة
  ...
  const fetchBranches = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.warn('branches API did not return an array:', data);
        setBranches([]);
      }
    } catch (err) {
      toast.error('فشل تحميل الفروع');
      setBranches([]); // ✅ تجنب الأخطاء لاحقًا
    }
  };
  ...
  const fetchCollections = async () => {
    try {
      ...
      if (res.data && Array.isArray(res.data.data)) {
        ...
        console.warn('Unexpected collections response format:', res.data);
        setCollections([]);
      }
    } catch (err) {
          ...
          <table className="min-w-full bg-white text-sm border">
            <thead>
              <tr className="bg-gray-200 text-center">
                ...
                <th className="px-2 py-2">الكاشير</th>
              ...
              {collections.length === 0 ? (
                ...
                collections.map((collection, index) => (
                  <tr key={collection.id} className="text-center border-b">
                    ...
                    <td className="px-2 py-2">{collection.is_verified ? '✔️' : '⏳'}</td>
                  </tr>
                ))
              )}
            </tbody>
