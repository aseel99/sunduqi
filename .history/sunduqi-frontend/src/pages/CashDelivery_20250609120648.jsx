...
export default function CashDelivery() {
  ...
  const fetchMatchedTotal = async () => {
    try {
      const res = await axios.get('/cash-deliveries/matched-confirmed', {
        params: {
          user_id: user.id,
      ...
      toast.error('لا يوجد مجموع مطابق لهذا التاريخ.');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        '/cash-deliveries',
        { date, amount: matchedAmount, notes },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-4 bg-white rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {matchedAmount !== null && (
          <div>
            <label className="block mb-1">المبلغ المطابق</label>
            <input
              type="text"
