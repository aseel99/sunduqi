...
export default function UserManagement() {
  ...
  const handleResetPassword = async (userId) => {
    ...
    try {
      ...
      toast.success(`تمت إعادة تعيين كلمة المرور. كلمة المرور الجديدة: ${res.data.newPassword}`);
    } catch {
  ...
  const handleAddBranch = async (e) => {
    e.preventDefault();
    if (!branchName.trim()) return toast.error('يرجى إدخال اسم الفرع');
    try {
      await axios.post('/branches', { name_ar: branchName });
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-xl shadow p-6">
        <div className="flex gap-4 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-semibold ${tab === 'users' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            إدارة المستخدمين
          </button>
          <button
            onClick={() => setTab('branches')}
            className={`px-4 py-2 rounded-md text-sm font-semibold ${tab === 'branches' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            إدارة الفروع
          </button>
        </div>
        ...
        {tab === 'users' ? (
            ...
            <form onSubmit={handleSubmit} className="mb-8 p-4 border rounded-lg bg-gray-50 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" minLength={3} required className="w-full p-2 border rounded" value={formData.username} onChange={(e) => setFormData({ ...formData, username: e.target.value })} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select className="w-full p-2 border rounded" value={formData.branch_id} onChange={(e) => setFormData({ ...formData, branch_id: e.target.value })} required>
                ...
                <div>
                  ...
                  <input type="password" minLength={6} required className="w-full p-2 border rounded" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} />
                </div>
                <div className="flex items-center pt-6">
                  <input type="checkbox" id="is_active" checked={formData.is_active} onChange={(e) => setFormData({ ...formData, is_active: e.target.checked })} className="ml-2" />
                  <label htmlFor="is_active" className="text-sm">مفعل</label>
                </div>
              </div>
              <button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded disabled:bg-blue-300">
                  ...
                  {users.map(user => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="p-2 border space-x-2 space-x-reverse">
                        <button onClick={() => toggleUserStatus(user.id, user.is_active)} className={`px-2 py-1 text-xs rounded ${user.is_active ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                          {user.is_active ? 'تعطيل' : 'تفعيل'}
                        </button>
                        <button onClick={() => handleResetPassword(user.id)} className="px-2 py-1 text-xs bg-yellow-100 text-yellow-600 rounded">
                          إعادة تعيين
                        </button>
                      </td>
                    </tr>
          ...
          <form onSubmit={handleAddBranch} className="space-y-4">
            <h2 className="text-2xl font-bold text-blue-700 mb-6">إضافة فرع جديد</h2>
            <div>
              <label className="block mb-1 text-sm">اسم الفرع</label>
              <input type="text" className="w-full border p-2 rounded" value={branchName} onChange={(e) => setBranchName(e.target.value)} />
            </div>
            <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">إضافة الفرع</button>
          </form>
        )}
      </div>
    </div>
