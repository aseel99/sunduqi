...
export default function Matching() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        if (user.role === 'admin') {
          const { data } = await axios.get('/cash-matching/branch_totals');
          setAllBranches(data);
        } else {
          const [summaryRes, matchedRes] = await Promise.all([
            axios.get(`/cash-matching/matching-summary?user_id=${user.id}&branch_id=${user.branch_id}`),
            axios.get('/cash-deliveries/matched-total', {
              params: { user_id: user.id, branch_id: user.branch_id, date: today },
              headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            })
          ]);
          setSummary(summaryRes.data);
          setMatchedTotal(matchedRes.data.matched_total);
        }
      } catch (err) {
  ...
  const handleAdminSubmit = async (e) => {
    ...
    try {
      await Promise.all(
        allBranches.map(branch => axios.post('/cash-matching/admin-confirm', {
          user_id: user.id,
          branch_id: branch.branch_id,
          expected_total: branch.expected_total,
          actual_total: parseFloat(actualTotals[branch.branch_id]),
          notes: notesMap[branch.branch_id] || '',
      ...
      <div className="min-h-screen bg-gray-100 p-6 text-right">
        <div className="max-w-6xl mx-auto bg-white p-6 rounded-xl shadow">
          ...
          <form onSubmit={handleAdminSubmit}>
                    ...
                    <tr key={branch.branch_id}>
                      ...
                      <td className="p-2 border">
                        <input
                          ...
                          required
                          value={actual}
                          onChange={(e) => setActualTotals({ ...actualTotals, [branch.branch_id]: e.target.value })}
                      ...
                      <td className="p-2 border">
                        <textarea
                          ...
                          onChange={(e) => setNotesMap({ ...notesMap, [branch.branch_id]: e.target.value })}
                          className="w-full p-1 border rounded"
                        />
                      </td>
                    </tr>
            ...
            <button className="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700">حفظ المطابقات</button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-100 p-6 text-right">
      <div className="max-w-md mx-auto bg-white p-6 rounded-xl shadow">
        ...
        <form onSubmit={handleCashierSubmit}>
          ...
          />
          <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700">حفظ المطابقة</button>
        </form>
