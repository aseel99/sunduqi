...
export default function AdminCollection() {
  ...
  const handleCollect = async (group) => {
    ...
    try {
      ...
      window.location.reload(); // ✅ تحديث كامل بعد الاستلام
    } catch (error) {
    ...
    <div className="p-4">
      ...
      {groups.length === 0 ? (
        ...
        groups.map((group, idx) => (
          <div
            key={idx}
            className="border p-4 mb-3 rounded shadow bg-white flex justify-between items-center"
          >
            <div>
              <p><strong>المستخدم:</strong> {group.user?.full_name}</p>
              <p><strong>الفرع:</strong> {group.branch?.name_ar}</p>
              <p><strong>المبلغ:</strong> {group.total_delivered} د.أ</p>
            </div>
