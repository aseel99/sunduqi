...
export default function AdminSummary() {
  ...
  const fetchSummary = async () => {
    try {
      ...
      console.log('Branch Summary Data:', res.data);
      setBranchSummaries(res.data.data); // استخراج فقط المصفوفة
    } catch (err) {
            ...
            branchSummaries.map(row => {
              ...
              return (
                <tr key={`${row.branch_id}-${row.date}`}>
                  ...
                  <td className="border p-2">{parseFloat(row.total_receipts).toFixed(2)}</td>
                  <td className="border p-2">{parseFloat(row.total_disbursements).toFixed(2)}</td>
                  <td className="border p-2">{parseFloat(row.total_balance).toFixed(2)}</td>
                  <td className="border p-2">{parseFloat(row.total_transferred).toFixed(2)}</td>
