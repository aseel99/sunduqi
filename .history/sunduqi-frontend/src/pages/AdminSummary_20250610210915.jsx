        ...
        <tbody>
          {Array.isArray(branchSummaries) && branchSummaries.length > 0 ? (
            branchSummaries.map(row => {
              ...
              const transferred = parseFloat(row.total_transferred) || 0;
                ...
                <tr key={`${row.branch_id}-${date}`}>
                  ...
                  <td className="border p-2">
                    <button
                      ...
                      disabled={!transferAmounts[row.branch_id] || parseFloat(transferAmounts[row.branch_id]) <= 0 || parseFloat(transferAmounts[row.branch_id]) > remaining}
