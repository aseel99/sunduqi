  ...
  const renderLink = (link, isMobile = false) => (
    <Link
      ...
      className={`flex items-center px-4 py-2 rounded-md transition font-medium ${
        isActive(link.path)
          ? 'bg-yellow-300 text-[#1c1c1c]'
          : 'text-white hover:bg-[#0599ba]'
    ...
    <div key="submenu" className="space-y-1">
      <button
        ...
        className={`w-full flex justify-between items-center px-4 py-2 rounded-md font-medium transition ${
          submenuOpen ? 'bg-yellow-300 text-[#1c1c1c]' : 'text-white hover:bg-[#0599ba]'
        } ${isMobile ? 'text-base' : 'text-sm'}`}
      >
  ...
  return (
    <div className="min-h-screen flex font-[Tajawal]" dir="rtl">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#047291] text-white shadow-[0_0_12px_#8e8e8e]">
        <div className="flex items-center justify-center h-20 bg-yellow-400 border-b border-yellow-300">
          <img src={logo} alt="شعار" className="h-10 w-auto ml-2" />
          <span className="text-xl font-bold text-[#1c1c1c]">صندوقي</span>
        </div>
      ...
      {/* Sidebar - Mobile */}
      <div className={`fixed inset-0 z-50 transition-transform duration-300 md:hidden bg-[#047291] text-white ${mobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-4 py-4 bg-yellow-400 border-b border-yellow-300">
          <div className="flex items-center">
            <img src={logo} alt="شعار" className="h-8 w-auto ml-2" />
            <span className="text-lg font-bold text-[#1c1c1c]">صندوقي</span>
          </div>
          <button onClick={() => setMobileOpen(false)}>
            <svg className="h-6 w-6 text-[#1c1c1c]" fill="none" stroke="currentColor">
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col bg-[#f4f4f4] min-h-screen">
        <header className="md:hidden bg-white border-b p-4 flex justify-between items-center shadow-sm">
          <button onClick={() => setMobileOpen(true)} className="text-gray-800">
