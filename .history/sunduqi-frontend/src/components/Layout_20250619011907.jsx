...
function Layout() {
  ...
  const renderLink = (link, isMobile = false) => (
    <Link
      ...
      className={`sidebar-link ${isActive(link.path) ? 'bg-[#2e3e50] text-white' : isMobile ? 'text-[#1A237E]' : 'text-white'} ${isMobile ? 'text-base' : 'text-sm'} flex items-center gap-2 px-3 py-2 rounded`}
    >
      <span>{link.icon}</span>
      <span>{link.name}</span>
