...
function Layout() {
  ...
  const renderLink = (link, isMobile = false) => (
    <Link
      ...
      className={`sidebar-link ${isActive(link.path) ? 'sidebar-link-active' : 'sidebar-link-inactive'} ${isMobile ? 'text-base' : 'text-sm'}`}
    >
  ...
  const renderSubmenu = (isMobile = false) => (
    <div key="submenu" className="space-y-1">
      <button
        ...
        className={`sidebar-link w-full justify-between ${submenuOpen ? 'sidebar-link-active' : 'sidebar-link-inactive'} ${isMobile ? 'text-base' : 'text-sm'}`}
      >
  ...
  return (
    <div className="min-h-screen flex bg-[#f4f4f4] text-[#1c1c1c] font-[Tajawal]" dir="rtl">
      {/* Sidebar - Desktop */}
      <aside className="hidden md:flex flex-col w-64 bg-[#047291] text-white">
        <div className="sidebar-header">
          <img src={logo} alt="شعار" className="h-10 w-auto mx-auto mb-1" />
          <div className="app-name">صندوقي</div>
          <div className="app-sub">نظام إدارة الكاش</div>
        </div>
      ...
      <div className={`fixed inset-0 bg-white z-50 transition-transform duration-300 md:hidden ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-4 py-4 bg-[#047291] text-white">
