...
export default function Layout() {
  ...
  const navLinks = [
    ...
    { type: 'submenu', name: 'تفاصيل الكاش والسندات', icon: '📂', allowedRoles: ['admin'], children: submenuLinks },
  ...
  const filteredLinks = navLinks.filter(link =>
    link.allowedRoles?.includes(user?.role) || link.type === 'submenu'
  );
    ...
    <div className="space-y-1">
      <button
        ...
        className={`w-full flex justify-between items-center px-4 py-2 rounded-md font-medium ${
          submenuOpen ? 'bg-[#FFD54F] text-[#212121]' : 'text-gray-700 hover:bg-gray-100'
        } ${isMobile ? 'text-base' : 'text-sm'}`}
      >
        <span className="flex items-center">
          <span className="ml-2">📂</span> تفاصيل الكاش والسندات
        </span>
        <svg className={`h-4 w-4 transform ${submenuOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor">
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] text-[#212121] font-[Tajawal] flex">
      {/* Sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-white border-r h-screen sticky top-0 z-40 shadow-sm">
        <div className="flex items-center justify-center h-20 border-b px-4 bg-[#FFD54F]">
          <img src={logo} alt="صندوقي" className="h-10 w-auto mr-2" />
          <span className="text-xl font-bold text-[#212121]">صندوقي</span>
        </div>
      ...
      <div className={`fixed inset-0 bg-white z-50 transition-transform duration-300 md:hidden ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-4 py-4 border-b bg-[#FFD54F]">
          <div className="flex items-center">
            <img src={logo} alt="شعار" className="h-8 w-auto mr-2" />
            <span className="text-lg font-bold text-[#212121]">صندوقي</span>
          </div>
          <button onClick={() => setMobileMenuOpen(false)}>
            <svg className="h-6 w-6 text-gray-600" fill="none" stroke="currentColor">
