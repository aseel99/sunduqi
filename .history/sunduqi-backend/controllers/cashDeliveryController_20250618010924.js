...
exports.closeCashboxOnly = async (req, res) => {
  try {
    ...
    // جلب المطابقات المالية للمستخدم في هذا اليوم
    const matchings = await CashMatching.findAll({
      where: {
        user_id: userId,
        branch_id: branchId,
    ...
    for (const match of matchings) {
      // تحقق من عدم وجود تسليم أو إغلاق مسبق لنفس المطابقة
      ...
      if (existing) {
        return res.status(400).json({
          message: 'لا يمكنك إغلاق صندوق تم تسليمه أو تم إغلاقه مسبقًا'
        });
      }
      ...
      // تحديث حالة الافتتاح ليتم إغلاقه
