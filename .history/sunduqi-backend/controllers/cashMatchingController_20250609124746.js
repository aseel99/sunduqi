...
exports.getMatchingSummary = async (req, res) => {
  try {
    ...
    const receipts = await Receipt.findAll({ where: whereClause });
    const disbursements = await Disbursement.findAll({ where: whereClause });
    ...
    const expectedTotal = (openingBalance?.amount || 0) + totalReceipts - totalDisbursements;

    return res.status(200).json({
      total_receipts: totalReceipts.toFixed(2),
      total_disbursements: totalDisbursements.toFixed(2),
      opening_balance: openingBalance?.amount || 0,
      expected_total: expectedTotal.toFixed(2),
      date: openingBalance?.date || new Date(),
    });

  } catch (error) {
    console.error('Error in getMatchingSummary:', error);
    res.status(500).json({ message: 'فشل في تحميل بيانات المطابقة' });
  }
};
...
exports.getMatchingSummary = async (req, res) => {
  try {
    ...
    // Get the latest opening balance before or on the day
    const openingBalance = await OpeningBalance.findOne({
      where: {
        user_id,
        branch_id,
    ...
    const totalReceipts = receipts.reduce((sum, r) => sum + parseFloat(r.amount), 0);
    const totalDisbursements = disbursements.reduce((sum, d) => sum + parseFloat(d.amount), 0);
    const openingAmount = openingBalance ? parseFloat(openingBalance.amount) : 0;
    ...
    console.error('Error in getMatchingSummary:', error);
    return res.status(500).json({ message: 'فشل في حساب المجاميع', error: error.message });
  }
};
