...
exports.closeCashboxOnly = async (req, res) => {
  try {
    ...
    for (const match of approvedMatchings) {
      // ✅ 2. التحقق من وجود تسليم لنفس المطابقة وتاريخ اليوم
      const existingDelivery = await CashDelivery.findOne({
        where: {
          user_id: userId,
          branch_id: branchId,
          opening_balance_id: match.opening_balance_id,
          date: today // ✅ تحقق دقيق للتاريخ
        }
      });

      // ✅ 3. منع الإغلاق إذا كان قد تم التسليم فعلاً اليوم
      if (existingDelivery) {
        if (existingDelivery.is_verified || !existingDelivery.is_closed_only) {
          return res.status(400).json({ message: 'لا يمكنك إغلاق صندوق تم تسليمه مسبقًا' });
        }
        // ✅ تجاوز إذا تم الإغلاق بالفعل اليوم بدون تسليم
        continue;
      }
      ...
      // ✅ 5. إنشاء سجل التسليم بدون تسليم فعلي
      const newDelivery = await CashDelivery.create({
