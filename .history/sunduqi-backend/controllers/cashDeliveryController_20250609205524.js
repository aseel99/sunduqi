...
//     const match = await require('../models').CashMatching.findOne({
...
//     res.status(200).json({ total: match.actual_total });
//   } catch (error) {
...
exports.createDelivery = async (req, res) => {
  try {
    ...
    const delivery = await CashDelivery.create({
      ...
      branch_id,
      opening_balance_id,
      is_verified: true, // ✅ تأكيد التسليم مباشرةً
      verified_by: user_id,
      verified_at: new Date()
    });
...
// controllers/cashDeliveryController.js
...
exports.getPendingDeliveries = async (req, res) => {
  try {
    ...
    const pending = await CashDelivery.findAll({
      where: {
        is_verified: true,
        is_collected: false,
        date: { [Op.lt]: today } // فقط الأيام السابقة
      },
      include: [
        {
          model: User,
    ...
    const formatted = pending.map((delivery) => ({
      id: delivery.id,
      type: 'delivery',
      branch_name: delivery.branch?.name_ar || '',
      user_name: delivery.user?.full_name || '',
      amount: delivery.delivered_amount,
    ...
    res.status(500).json({ message: 'حدث خطأ أثناء تحميل التسليمات المتأخرة', error: error.message });
  }
};
