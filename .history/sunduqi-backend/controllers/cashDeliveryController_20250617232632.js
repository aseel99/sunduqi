...
exports.closeCashboxOnly = async (req, res) => {
  try {
    ...
    // ✅ 1. الحصول على جميع المطابقات المالية لليوم الحالي
    const approvedMatchings = await CashMatching.findAll({
      where: {
        user_id: userId,
        branch_id: branchId,
        date: today,
        // is_resolved: true ← أضفه إن أردت فقط المعتمدة
      }
    });
    ...
    for (const match of approvedMatchings) {
      ...
      // ✅ 3. إنشاء سجل التسليم بدون تسليم فعلي
      const newDelivery = await CashDelivery.create({
      ...
      // ✅ 4. تحديث حالة الافتتاح المرتبط ليتم إغلاقه
...
exports.closeCashboxOnly = async (req, res) => {
  try {
    ...
    // 1️⃣ جلب كل المطابقات لليوم الحالي
    const approvedMatchings = await CashMatching.findAll({
      where: {
        user_id: userId,
        branch_id: branchId,
    ...
    for (const match of approvedMatchings) {
      // 2️⃣ تحقق من وجود تسليم أو إغلاق سابق لهذه المطابقة
      const existingDelivery = await CashDelivery.findOne({
        where: {
          opening_balance_id: match.opening_balance_id,
          user_id: userId,
          branch_id: branchId,
      ...
      if (existingDelivery) {
        // ⛔ لا يسمح بالإغلاق بعد التسليم
        if (!existingDelivery.is_closed_only && existingDelivery.is_verified) {
          return res.status(400).json({
            message: 'لا يمكنك إغلاق صندوق تم تسليمه مسبقًا'
          });
        }

        // ✅ إذا كان مغلقاً بالفعل بدون تسليم، لا نكرر الإنشاء
        if (existingDelivery.is_closed_only && !existingDelivery.is_verified) {
          continue;
        }
      }

      // 3️⃣ إنشاء رقم تسليم فريد
      const deliveryNumber = `CL-${Date.now().toString().slice(-8)}-${Math.floor(Math.random() * 10)}`;

      // 4️⃣ إنشاء تسليم مغلق فقط
      const newDelivery = await CashDelivery.create({
        delivery_number: deliveryNumber,
        user_id: userId,
        branch_id: branchId,
    ...
    if (deliveries.length === 0) {
      return res.status(400).json({ message: 'جميع المطابقات مغلقة مسبقًا أو تم تسليمها' });
    }
