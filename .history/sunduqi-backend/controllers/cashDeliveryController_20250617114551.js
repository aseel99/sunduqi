...
exports.closeCashboxOnly = async (req, res) => {
  try {
    ...
    // Get total matched amount for today (same logic from your matching summary)
    const totalMatched = await CashMatching.sum('total', {
      where: {
        user_id: userId,
        branch_id: branchId,
        date: today,
    ...
    if (!totalMatched || totalMatched <= 0) {
      return res.status(400).json({ message: 'No matched amount available to close.' });
    }
    ...
    if (existing) {
      return res.status(400).json({ message: 'Cashbox already closed or delivered for today.' });
    }

    // Create delivery record with is_closed_only = true
    const delivery = await CashDelivery.create({
      delivery_number: `CLSD-${Date.now()}`,
      user_id: userId,
      branch_id: branchId,
      delivered_amount: totalMatched,
      date: today,
      is_verified: false,
      is_collected: false,
      is_closed_only: true
    ...
    res.status(200).json({ message: 'Cashbox closed without delivery', delivery });
  } catch (err) {
    console.error('Error closing cashbox:', err);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
};

exports.deliverAfterClosure = async (req, res) => {
  try {
    const userId = req.user.id;
    const branchId = req.user.branch_id;
    const today = new Date().toISOString().split('T')[0];

    // Find record with is_closed_only = true
    const record = await CashDelivery.findOne({
      where: {
        user_id: userId,
        branch_id: branchId,
        date: today,
    ...
    if (!record) {
      return res.status(400).json({ message: 'No closed-only record found for today.' });
    }

    // Update the record to reflect full delivery
    record.is_closed_only = false;
    record.is_verified = true;
    record.verified_at = new Date();

    await record.save();

    res.status(200).json({ message: 'Cash successfully delivered after closure.', record });
  } catch (err) {
    console.error('Error delivering after closure:', err);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
};
