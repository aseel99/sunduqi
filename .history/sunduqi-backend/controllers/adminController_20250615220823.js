...
exports.getCasherStats = async (req, res) => {
  try {
    ...
    // احسب المجاميع الخاصة بفرع الكاشير
    const totalReceipts = await Receipt.sum('amount', {
      where: { branch_id }
    });

    const totalDisbursements = await Disbursement.sum('amount', {
      where: { branch_id }
    });

    const totalDeliveries = await CashDelivery.sum('delivered_amount', {
      where: { branch_id }
    });

    const totalCollections = await CashCollection.sum('total_collected', {
      where: { branch_id }
    });
    ...
    console.error('Error fetching casher dashboard stats:', error);
    res.status(500).json({ message: 'Failed to fetch casher stats' });
  }
};
