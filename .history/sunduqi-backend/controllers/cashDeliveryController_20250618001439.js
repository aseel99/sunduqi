  ...
  try {
    ...
    const approvedMatchings = await CashMatching.findAll({
      where: {
        ...
        branch_id: branchId,
        date: today,
        // يمكنك تفعيل شرط is_resolved إذا أردت حصرها بالموافق عليها فقط
        // is_resolved: true
      }
    });
    ...
    for (const match of approvedMatchings) {
      // ✅ تحقق إن كان يوجد تسليم سابق لهذا المطابقة
      ...
      if (existingDelivery) {
        ...
        // ✅ تعديل السجل الحالي بدلاً من إنشاء واحد جديد
        ...
        // ✅ إنشاء سجل جديد إن لم يكن موجودًا
