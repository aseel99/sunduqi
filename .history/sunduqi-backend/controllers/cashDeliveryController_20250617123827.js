...
exports.deliverClosedCash = async (req, res) => {
  try {
    ...
    const pendingDeliveries = await CashDelivery.findAll({
      where: {
        user_id: userId,
        is_closed_only: true,
        is_verified: false
    ...
    if (pendingDeliveries.length === 0) {
      return res.status(400).json({ message: 'لا توجد عهدة مغلقة قيد التسليم' });
    }
    ...
    res.status(200).json({ message: 'تم تسليم العهدة المغلقة بنجاح', count: pendingDeliveries.length });
  } catch (error) {
