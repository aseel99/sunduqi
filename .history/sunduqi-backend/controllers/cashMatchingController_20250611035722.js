...
exports.getAllMatchings = async (req, res) => {
  try {
    const {
      ...
      fromDate,
      toDate,
      date // ✅ تاريخ يوم واحد (مثل اليوم الحالي)
    } = req.query;
    ...
    // ✅ فلترة حسب المستخدم: admin يقدر يختار user_id، غيره فقط نفسه
    if (req.user.role === 'admin') {
      if (user_id) {
        where.user_id = user_id;
      }
    ...
    const matchings = await CashMatching.findAndCountAll({
      ...
      include: [
        { model: Branch, as: 'branch', attributes: ['id', 'name_ar'] },
        {
          ...
          required: true,
          where: { role: 'casher' } // 👈 فلترة الكاشير فقط
        },
        { model: User, as: 'resolver', attributes: ['id', 'full_name'] }
      ],
      order: [['date', 'DESC']],
