...
exports.createMatching = async (req, res) => {
  try {
    ...
    // check there is opening
    const existing = await CashMatching.findOne({
      where: { opening_balance_id: opening.id }
    });

    if (existing) {
      return res.status(400).json({ message: '⚠️ تمت المطابقة مسبقًا لهذا الافتتاح' });
    }
    ...
    const match = await CashMatching.create({
      ...
      actual_total: expectedTotal.toFixed(2),
...
exports.getOpenSessionSummary = async (req, res) => {
  try {
    const user_id = req.user.id;
    const branch_id = req.user.branch_id;
    ...
    const totalReceipts = receipts.reduce((sum, r) => sum + parseFloat(r.amount), 0);
    const totalDisbursements = disbursements.reduce((sum, d) => sum + parseFloat(d.amount), 0);
    const openingBalance = parseFloat(opening.amount);
    const expectedTotal = openingBalance + totalReceipts - totalDisbursements;
    ...
    console.error('❌ Error in getOpenSessionSummary:', error);
    res.status(500).json({ message: 'فشل في حساب ملخص الجلسة', error: error.message });
  }
};
