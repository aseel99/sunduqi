...
exports.createCollection = async (req, res) => {
  try {
    const {
      ...
      notes,
      user_id,
      delivery_id // ✅ جديد: نحتاجه لتحديث السند المحدد فقط
    } = req.body;

    const collected_by = req.user.id; // المدير أو من قام بالاستلام
    const collection_number = await generateCollectionNumber();

    // ✅ التحقق من وجود delivery_id
    if (!delivery_id) {
      return res.status(400).json({ message: 'delivery_id مطلوب' });
    }

    const delivery = await CashDelivery.findByPk(delivery_id);
    if (!delivery || delivery.is_collected) {
      return res.status(404).json({ message: 'لم يتم العثور على السند أو تم استلامه مسبقًا' });
    }
