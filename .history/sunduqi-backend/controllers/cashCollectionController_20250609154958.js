...
exports.createCollection = async (req, res) => {
  try {
    const { total_amount, branch_id, user_id } = req.body;
    const admin_id = req.user.id;
    const collection_number = await generateCollectionNumber(); // مولد رقم الاستلام
    const collection_date = new Date(); // تاريخ اليوم
    ...
    res.status(500).json({ message: 'فشل في إنشاء استلام الكاش', error: error.message });
  }
};
